...
it('can stream text with a prompt', function (): void {
    FixtureResponse::fakeStreamResponse('converse-stream', 'converse/stream-text-1.txt');

    $events = [];
    $generator = (new Prism)->text()
        ->using('bedrock', 'us.amazon.nova-lite-v1:0')
    ...
    // Verify we received the expected events (8 total)
...
it('can stream text with tool calls', function (): void {
    FixtureResponse::fakeStreamResponse('converse-stream', 'converse/stream-with-tool-call-1.txt');
...
it('handles streaming errors', function (): void {
    FixtureResponse::fakeStreamResponse('converse-stream', 'converse/stream-error-1.txt');
    ...
    expect($errorEvent->errorType)->toBe('unknown_error');
    expect($errorEvent->message)->toBe('Internal server error occurred');
